...
def watch(
    ...
    while True:
        ...
        cycle_id = cycle_state.cycle_id if cycle_state is not None else uuid4().hex
        cycle_started_at = datetime.now(ZoneInfo(timezone_name)).isoformat()
        cycle_topics_fetched = 0
        cycle_topics_entered = 0
        cycle_posts_fetched = 0
        cycle_notifications_sent = 0
        cycle_success = True
        cycle_error = ""
        if store is not None:
            store.begin_watch_cycle(cycle_id, cycle_started_at)
        ...
        try:
            if notifier and (now - last_notify_ts) >= notify_interval_secs:
                try:
                    cycle_notifications_sent += _poll_notifications(
                        client,
                        store,
                        notifier,
            ...
            try:
                ...
                cycle_success = False
                cycle_error = "latest/unseen timeout"
            ...
            cycle_topics_fetched = len(topics)
            if plugin_manager is not None and cycle_state is not None:
            ...
            for topic in topics:
                ...
                if topic_id:
                    ...
                    try:
                        ...
                        cycle_posts_fetched += int(result.fetched_post_count or 0)
            ...
            if store is not None:
                store.finish_watch_cycle(
                    cycle_id,
                    ended_at=datetime.now(ZoneInfo(timezone_name)).isoformat(),
                    topics_fetched=cycle_topics_fetched,
                    topics_entered=cycle_topics_entered,
                    posts_fetched=cycle_posts_fetched,
                    notifications_sent=cycle_notifications_sent,
                    success=cycle_success,
                    error_text=cycle_error,
                )
...
def _touch_topic(
    ...
    if highest > 0 and highest <= last_synced_post_number:
        store.upsert_topic_crawl_state(topic_id=topic_id, last_synced_post_number=last_synced_post_number, last_stream_len=len(stream_ids) if stream_ids else 0)
        return TopicTouchResult(
            remaining_posts=remaining_posts,
            topic=topic,
            content_posts=content_posts,
    ...
    if not missing_all:
        store.upsert_topic_crawl_state(topic_id=topic_id, last_synced_post_number=highest, last_stream_len=len(stream_ids))
        return TopicTouchResult(
            remaining_posts=remaining_posts,
            topic=topic,
            content_posts=content_posts,
    ...
    store.upsert_topic_crawl_state(topic_id=topic_id, last_synced_post_number=last_synced_post_number, last_stream_len=len(stream_ids))
    return TopicTouchResult(
        remaining_posts=remaining_posts,
        topic=topic,
        content_posts=content_posts,
...
def _log_watch_summary(store: StoreBackend | None) -> None:
    if store is None:
        return
