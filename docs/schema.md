...
- 即使如此仍建议保留，便于统一 `sqlite` / `postgres` schema。
...
- post 默认不做 snapshots。
- 同一 post 后续如再次抓到，可做 upsert 覆盖当前值，但不单独保留历史。
- `fetched_at` 表示本地第一次或本次抓取到该 post 的时间，具体语义在实现时需固定为一种。
  - 推荐定义为“首次写入时间”。
...
`topic_snapshots.raw_json` 建议保留裁剪后的 `get_topic()` 返回值：
...
这一部分可以做，而且建议按“逻辑表级导出导入”处理，不做数据库文件级转换。
...
1. `discorsair export --format ndjson`
2. `discorsair import --input ./export --backend postgres`
